...
Реализуйте сервис notification-processor, который читает события пользовательских действий из Kafka,
формирует персонализированные уведомления и гарантирует их доставку через внешний сервис.
...
По понятным причинам делать саму отправку не надо, достаточно вывести лог, в котором будет указано
кому было отправлено сообщение и по какому каналу

Так же реализовать функцию эмуляции сбоя доставки таким образом, что 10% вызовов возвращают
временные ошибки (повторить через экспоненциальную задержку), 1% вызовов возвращают перманентные
ошибки (прекратить попытки после 3-й)
...
- Exactly-once семантика — сообщение не теряется при падении сервиса между записью в БД и коммитом
  оффсета.
...
- Используйте Kafka (любая популярная библиотека на Go) с ручным управлением оффсетами (автокоммит
  запрещён).
